...
begin
  ...
  select coalesce(sum(net_payable),0) into payroll_costs
  from public.payroll_periods where year=c.year and month=c.month and status<>'cancelled';

  period_result:=accrual_revenue-manual_costs-vendor_costs-payroll_costs;
